Given(/^I verify user level properties for "([^"]*)"$/) do |affiliation|
  ...
  case affiliation
  when "Government"
    levels.push(elements[0].text.to_i, elements[3].text.to_i, elements[6].text.to_i)
    puts "Actual User Property Levels: #{levels}"
    puts "Expected User Property Levels: #{IVAGovernment.new.user_property_levels}"
